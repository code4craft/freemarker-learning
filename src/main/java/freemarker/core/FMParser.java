 ...
 */
  final public 
Expression Expression() throws ParseException {Expression exp;
    exp = OrExpression();
{if ("" != null) return exp;}
...
ExistsExpression result = new ExistsExpression(exp);
  ...
  final public Expression BuiltIn(Expression exp) throws ParseException {Token t=null;
...
NumericalOutput result;
      if (fmt != null) {
         ...
         if (maxFrac == -1) {
            if (minFrac == -1) {
               {if (true) throw new ParseException("Invalid format specification, at least one of m and M must be specified!", template, fmt);}
            }
            maxFrac = minFrac;
         } else if (minFrac == -1) {
            minFrac = 0;
         ...
         if (minFrac > maxFrac) {
            {if (true) throw new ParseException("Invalid format specification, min cannot be greater than max!", template, fmt);}
         }
...
RecoveryBlock result = new RecoveryBlock(block);
  ...
  final public IteratorBlock List() throws ParseException {Expression exp;
...
if (inMacro) {
         ...
         if (exp == null) {
            {if (true) throw new ParseException("A return instruction can only occur inside a macro or function", template, start);}
         }
      }
...
ass = new Assignment(varName, exp, scope);
      ...
      case IN:{
        ...
        nsExp = Expression();
if (scope != Assignment.NAMESPACE) {if (true) throw new ParseException("Cannot assign to namespace here.", template, id);}
        break;
        }
      default:
        jj_la1[34] = jj_gen;
...
if (scope != Assignment.LOCAL) {if (true) throw new ParseException("Mismatched assignment tags.", template, end);}
        break;
        }
      case END_ASSIGN:{
        end = jj_consume_token(END_ASSIGN);
if (scope != Assignment.NAMESPACE) {if (true) throw new ParseException("Mismatched assignment tags.", template, end);}
        break;
        }
      case END_GLOBAL:{
        end = jj_consume_token(END_GLOBAL);
if (scope != Assignment.GLOBAL) {if (true) throw new ParseException("Mismatched assignment tags", template, end);}
        break;
        }
      default:
        jj_la1[36] = jj_gen;
...
String attString = att.image;
             if (attString.equalsIgnoreCase("parse")) {
                ...
                encodingExp = exp;
                 ...
                 {if (true) throw new ParseException("Expecting parse= or encoding= to be specified.", template, att);}
             }
    }
    end = LooseDirectiveEnd();
...
if (caseIns.condition == null) {
           if (defaultFound) {
                {if (true) throw new ParseException("You can only have one default case in a switch statement", template, start);}
           }
           defaultFound = true;
        }
 ...
 */
  final public 
Token LooseDirectiveEnd() throws ParseException {Token t;
    switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
    case DIRECTIVE_END:{
 ...
 */
  final public 
TextBlock PCData() throws ParseException {StringBuffer buf = new StringBuffer();
...
if (stripText && contentNesting == 1)
             {if ("" != null) return TextBlock.EMPTY_BLOCK;}
 ...
 */
  final public 
Token UnparsedContent(StringBuffer buf) throws ParseException {Token t;
    label_18:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
      case KEEP_GOING:{
...
buf.setLength(buf.length() - t.image.length());
  ...
  final public TemplateElement Content() throws ParseException {MixedContent nodes = new MixedContent();
 ...
 */
  final public 
TemplateElement FreeMarkerText() throws ParseException {MixedContent nodes = new MixedContent();
     TemplateElement elem, begin = null;
    label_20:
    while (true) {
      switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
 ...
 */
  final public 
TemplateElement OptionalBlock() throws ParseException {TemplateElement tp = TextBlock.EMPTY_BLOCK;
    switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
    case ATTEMPT:
    case IF:
    case LIST:
...
doc.setParentRecursively(null);
       {if ("" != null) return doc.postParseCleanup(stripWhitespace);}
    throw new Error("Missing return statement in function");
  }
  ...
  private boolean jj_3R_176()
 {
  ...
  static {
   ...
   private static void jj_la1_init_3() {
      jj_la1_3 = new int[] {0x10a8000,0x10a880c,0x800,0xc,0xc,0x190,0x0,0xc0000003,0x0,0x1000000,0x28800,0x800,0xe00003,0x1e00033,0x0,0x0,0x5000,0x1000,0x5000,0x10a880c,0x2000,0x0,0x0,0x0,0x800000,0x10a880c,0x800000,0x0,0x0,0x0,0x0,0x0,0x0,0x1000,0x200000,0x200000,0x0,0x10200000,0x2000,0x1000000,0x0,0x20000,0x1000000,0x40,0x0,0x1000,0x40000,0x0,0x0,0x0,0x1000,0x0,0x0,0x1000000,0x2000,0x30000000,0x40000,0x1000000,0x10a980c,0x1000,0x10a880c,0x0,0x2000,0x1000000,0x30000000,0x0,0x0,0x0,0x30000000,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x1000000,0x0,0x1000,0x1000000,};
   }
